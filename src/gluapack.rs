...
use std::{collections::HashSet, convert::TryInto, path::{Path, PathBuf}};
...
const COMMENT_START: &'static [u8; 2] = &['-' as u8, '-' as u8];
...
async fn collect_lua_files<P: AsRef<Path>>(dir: P, patterns: &[GlobPattern], excludes: &[GlobPattern], entries: &[GlobPattern]) -> Result<(HashSet<LuaFile>, Vec<String>), PackingError> {
	...
	for pattern in patterns {
		for path in {
			glob(&dir.as_ref().join(pattern.as_str()).to_string_lossy())
				.expect("Failed to construct glob when joining addon directory")
				.filter(|result| {
					match result {
						Ok(path) => 'filter: loop {
							for exclude in excludes {
								if exclude.matches_path(path.strip_prefix(&dir).unwrap()) {
									break 'filter false;
								}
							}
							break 'filter true;
			...
			let fs_path = path?;
			let path = fs_path.strip_prefix(dir.as_ref()).unwrap().to_string_lossy().into_owned().replace('\\', "/");
			let tx = tx.clone();
...
async fn delete_old_gluapack_files<P: AsRef<Path>>(dir: P) -> Result<(), PackingError> {
	let gluapack_dir = glob(&dir.as_ref().join("gluapack/*").to_string_lossy()).unwrap()
	...
	let gluapack_loader = glob(&dir.as_ref().join("autorun/*_gluapack_*.lua").to_string_lossy())
		.unwrap()
		.take(1)
		.next();
...
async fn write_packed_chunks<P: AsRef<Path>, S: AsRef<str>>(dir: P, unique_id: S, bytes: Vec<u8>, chunk_name: &'static str) -> Result<(Vec<[u8; 20]>, usize), PackingError> {
	use tokio::io::AsyncWriteExt;

	let gluapack_dir = dir.as_ref().join(format!("gluapack/{}", unique_id.as_ref()));
...
async fn generate_cache_manifest<P: AsRef<Path>, S: AsRef<str>>(dir: P, unique_id: S, hashes_cl: Vec<[u8; 20]>, hashes_sh: Vec<[u8; 20]>) -> Result<(), PackingError> {
	...
	cache_manifest.push('}');
	tokio::fs::write(dir.as_ref().join(format!("gluapack/{}/manifest.lua", unique_id.as_ref())), cache_manifest).await?;

	Ok(())
}

async fn write_loader<P: AsRef<Path>, S: AsRef<str>>(dir: P, unique_id: S, sv_entry_files: Vec<String>, cl_entry_files: Vec<String>, sh_entry_files: Vec<String>) -> Result<(), PackingError> {
	...
	tokio::fs::write(dir.as_ref().join(format!("autorun/{}_gluapack_{}.lua", unique_id.as_ref(), env!("CARGO_PKG_VERSION"))), loader).await?;

	Ok(())
}

pub async fn gluapack(conf: Config, dir: PathBuf) -> Result<(usize, usize), PackingError> {
	println!("Collecting Lua files...");

	let ((sv, sv_entry_files), (cl, cl_entry_files), (sh, sh_entry_files)) = tokio::try_join!(
		collect_lua_files(&dir, &conf.include_sv, &conf.exclude, &conf.entry_sv),
		collect_lua_files(&dir, &conf.include_cl, &conf.exclude, &conf.entry_cl),
		collect_lua_files(&dir, &conf.include_sh, &conf.exclude, &conf.entry_sh),
	)?;
	...
	let (sv, cl, sh) = tokio::try_join!(
		tokio::task::spawn_blocking(move || pack_lua_files(sv, false)),
		tokio::task::spawn_blocking(move || pack_lua_files(cl, true)),
		tokio::task::spawn_blocking(move || pack_lua_files(sh, true))
	).expect("Failed to join threads");

	delete_old_gluapack_files(&dir).await?;

	let unique_id = conf.unique_id.as_ref().map(|x| x.to_owned()).unwrap_or_else(|| {
	...
	tokio::fs::create_dir_all(dir.join(&format!("gluapack/{}", &unique_id))).await.expect("Failed to create gluapack directory");

	if !sv.is_empty() {
		println!("Writing packed serverside files...");
		tokio::fs::write(dir.join(&format!("gluapack/{}/gluapack.sv.lua", &unique_id)), sv).await?;
	}
	...
	let total_packed_files = if !cl.is_empty() || !sh.is_empty() {
		...
		let ((hashes_cl, chunk_n_cl), (hashes_sh, chunk_n_sh)) = tokio::try_join!(
			write_packed_chunks(&dir, &unique_id, cl, "cl"),
			write_packed_chunks(&dir, &unique_id, sh, "sh"),
		)?;

		if !hashes_cl.is_empty() || !hashes_sh.is_empty() {
			println!("Generating clientside Lua cache manifest...");
			generate_cache_manifest(&dir, &unique_id, hashes_cl, hashes_sh).await?;
		}
	...
	write_loader(&dir, &unique_id, sv_entry_files, cl_entry_files, sh_entry_files).await?;
